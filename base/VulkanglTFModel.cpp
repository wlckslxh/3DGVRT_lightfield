...
{
#if SPLIT_BLAS
	for (auto cellVertices : splittedVertices) {
		vkDestroyBuffer(device->logicalDevice, cellVertices.buffer, nullptr);
		vkFreeMemory(device->logicalDevice, cellVertices.memory, nullptr);
	}
	for (auto cellIndices : splittedIndices) {
		vkDestroyBuffer(device->logicalDevice, cellIndices.buffer, nullptr);
		vkFreeMemory(device->logicalDevice, cellIndices.memory, nullptr);
	}
#else
...
void vkglTF::Model::loadNode(vkglTF::Node* parent, const tinygltf::Node& node, uint32_t nodeIndex, const tinygltf::Model& model, std::vector<uint32_t>& indexBuffer, std::vector<Vertex>& vertexBuffer, float globalscale)
{
...
#if DYNAMIC_SCENE
	const auto& extraValue = node.extras.Get<tinygltf::Value::Object>();
	{
		auto pair = extraValue.find("Instancing");
		if (pair != extraValue.end())
		{
			newNode->instancing = pair->second.Get<bool>();

			// If Instancing Objects, Get Generate Positions
			{
				auto pair = extraValue.find("Generate_Positions");
				if (pair != extraValue.end())
				{
					const auto& positionArr = pair->second.Get<tinygltf::Value::Array>();
					size_t arraySize = pair->second.ArrayLen();
					//newNode->generatePositions.resize(arraySize);

					for (uint32_t i = 0; i < (arraySize / 3); ++i)
					{
						float x = 0, y = 0, z = 0;
						x = (float)positionArr[i * 3 + 0].GetNumberAsDouble();
						y = (float)positionArr[i * 3 + 1].GetNumberAsDouble();
						z = (float)positionArr[i * 3 + 2].GetNumberAsDouble();

						newNode->generatePositions.push_back(glm::vec3(x, y, z));
					}					
				}
			}
		}	
	...
	{
		auto pair = extraValue.find("IsDynamic");
		if (pair != extraValue.end())
		{
			newNode->isStatic = !pair->second.Get<bool>();
		}
	}
...
*/
void vkglTF::Model::clipCell_1VB(std::vector<Vertex>& vertexBuffer, std::vector<uint32_t>& indexBuffer, std::unordered_map<Vertex, uint32_t>& uniqueVertices, std::vector<AABB_Triangle_Clipping::_AABB> gridAabb, int cellCnt, std::vector<std::vector<uint32_t>>& splittedIdx) {
	std::vector<int> errors;
	for (int i = 0; i < indexBuffer.size(); i += 3) {
		//Vertex* tri[3] = { &vertexBuffer[indexBuffer[i]], &vertexBuffer[indexBuffer[i + 1]], &vertexBuffer[indexBuffer[i + 2]] };
		...
		for (int cellIdx = 0; cellIdx < cellCnt; ++cellIdx) {
			...
			AABB_Triangle_Clipping::_clip_triangle_against_AABB_np(tri, gridAabb[cellIdx], polygon);

			if (polygon.size() == 0) continue;
			for (int k = 1; k < polygon.size() - 1; ++k) {

				if (uniqueVertices.count(polygon[0]) == 0) {
					uniqueVertices[polygon[0]] = static_cast<uint32_t>(vertexBuffer.size());
					vertexBuffer.push_back(polygon[0]);
				}
				splittedIdx[cellIdx].push_back(uniqueVertices[polygon[0]]);

				if (uniqueVertices.count(polygon[k]) == 0) {
					uniqueVertices[polygon[k]] = static_cast<uint32_t>(vertexBuffer.size());
					vertexBuffer.push_back(polygon[k]);
				}
				splittedIdx[cellIdx].push_back(uniqueVertices[polygon[k]]);

				if (uniqueVertices.count(polygon[k + 1]) == 0) {
					uniqueVertices[polygon[k + 1]] = static_cast<uint32_t>(vertexBuffer.size());
					vertexBuffer.push_back(polygon[k + 1]);
				}
...
void vkglTF::Model::saveGeometries_SBLAS_1VB(std::vector<Vertex>& vertexBuffer, std::vector<uint32_t>& indexBuffer, VkQueue transferQueue) {
	...
	for (int i = 0; i < vertexBuffer.size(); i++) {
		...
		if (vertexBuffer[i].pos.z > maxPos.z) maxPos.z = vertexBuffer[i].pos.z;
		if (vertexBuffer[i].pos.z < minPos.z) minPos.z = vertexBuffer[i].pos.z;
		if (uniqueVertices.count(vertexBuffer[i]) == 0) {
			uniqueVertices[vertexBuffer[i]] = i;
		}
	}
	...
	uint32_t numCellsTotal = numCells.x * numCells.y * numCells.z;

	std::vector<std::vector<uint32_t>> splittedIdx(numCellsTotal);

	//const glm::vec3 convertToIdx{ numCells.x / sceneSize.x, numCells.y / sceneSize.y , numCells.z / sceneSize.z };
	//glm::ivec3 gridIdx;
	...
	// split vertexes
	clipCell_1VB(vertexBuffer, indexBuffer, uniqueVertices, gridAabb, numCellsTotal, splittedIdx);

#if ADAPTIVE_SPLIT
	//additional split if many vertices
	int aabbIdx = 0;
	for (int i = 0; i < splittedIdx.size(); i++) {
		if (splittedIdx[i].size() > MAX_VERTEX_CNT) {
			std::cout << "adaptive split start - vertex count : " << splittedIdx[i].size() << "\n";
			std::vector<AABB_Triangle_Clipping::_AABB> adaptiveGridAabb(ADAPTIVE_SPLIT_CNT);
			glm::vec3 center = {
				(gridAabb[aabbIdx].xmin + gridAabb[aabbIdx].xmax) / 2,
				(gridAabb[aabbIdx].ymin + gridAabb[aabbIdx].ymax) / 2,
				(gridAabb[aabbIdx].zmin + gridAabb[aabbIdx].zmax) / 2
			};
			adaptiveGridAabb[0] = {
				gridAabb[aabbIdx].xmin, gridAabb[aabbIdx].ymin, gridAabb[aabbIdx].zmin,
				center.x, center.y, center.z
			};
			for (int j = 0; j < ADAPTIVE_SPLIT_CNT; j++) {
				if ((j & 0x1) == 0) {
					adaptiveGridAabb[j].xmin = gridAabb[aabbIdx].xmin;
					adaptiveGridAabb[j].xmax = center.x;
				}
				else {
					adaptiveGridAabb[j].xmin = center.x;
					adaptiveGridAabb[j].xmax = gridAabb[aabbIdx].xmax;
				}
				if ((j & 0x2) == 0) {
					adaptiveGridAabb[j].ymin = gridAabb[aabbIdx].ymin;
					adaptiveGridAabb[j].ymax = center.y;
				}
				else {
					adaptiveGridAabb[j].ymin = center.y;
					adaptiveGridAabb[j].ymax = gridAabb[aabbIdx].ymax;
				}
				if ((j & 0x4) == 0) {
					adaptiveGridAabb[j].zmin = gridAabb[aabbIdx].zmin;
					adaptiveGridAabb[j].zmax = center.z;
				}
				else {
					adaptiveGridAabb[j].zmin = center.z;
					adaptiveGridAabb[j].zmax = gridAabb[aabbIdx].zmax;
				}
			}

			std::vector<std::vector<uint32_t>> adaptiveSplittedIdx(ADAPTIVE_SPLIT_CNT);
			clipCell_1VB(vertexBuffer, splittedIdx[i], uniqueVertices, adaptiveGridAabb, ADAPTIVE_SPLIT_CNT, adaptiveSplittedIdx);

			splittedIdx.erase(splittedIdx.begin() + i);
			for (int j = ADAPTIVE_SPLIT_CNT - 1; j >= 0; j--) {
				splittedIdx.insert(splittedIdx.begin() + i, adaptiveSplittedIdx[j]);
			}
			i += 7;
		}
		aabbIdx++;
...
void vkglTF::Model::saveGeometries_SBLAS(std::vector<Vertex>& vertexBuffer, std::vector<uint32_t>& indexBuffer, VkQueue transferQueue) {
	/* split cells with aabb-triangle clipping */
	for (int i = 0; i < vertexBuffer.size(); i++) {
		if (vertexBuffer[i].pos.x > maxPos.x) maxPos.x = vertexBuffer[i].pos.x;
		if (vertexBuffer[i].pos.x < minPos.x) minPos.x = vertexBuffer[i].pos.x;
	...
	//splittedVertices.resize(numCellsTotal);
	//splittedIndices.resize(numCellsTotal);
	std::vector<std::vector<Vertex>> splittedVert(numCellsTotal);
	std::vector<std::vector<uint32_t>> splittedIdx(numCellsTotal);
	...
	std::vector<std::unordered_map<Vertex, uint32_t>> uniqueVertices(numCellsTotal);
	...
	for (int i = 0; i < numCellsTotal; i++) {
		...
		size_t vertexBufferSize = splittedVert[i].size() * sizeof(Vertex);
		size_t indexBufferSize = splittedIdx[i].size() * sizeof(uint32_t);
...
void vkglTF::Model::loadFromFile(std::string filename, vks::VulkanDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	saveGeometries_SBLAS_1VB(vertexBuffer, indexBuffer, transferQueue);
#else
#if HYBRID
	saveGeometries(vertexBuffer, indexBuffer, transferQueue);
#endif
	saveGeometries_SBLAS(vertexBuffer, indexBuffer, transferQueue);
#endif
#else
	saveGeometries(vertexBuffer, indexBuffer, transferQueue);
#endif
