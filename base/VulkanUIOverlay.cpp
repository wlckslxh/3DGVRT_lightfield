...
namespace vks 
{
...
#if defined(__ANDROID__)
		float scale = (float)vks::android::screenDensity / (float)ACONFIGURATION_DENSITY_MEDIUM * 0.8;
		io.FontGlobalScale = scale;
	...
	void UIOverlay::draw(const VkCommandBuffer commandBuffer, const vks::Buffer& vertexBuffer, const vks::Buffer& indexBuffer)
	{
		ImDrawData* imDrawData = ImGui::GetDrawData();
		int32_t vertexOffset = 0;
		int32_t indexOffset = 0;
