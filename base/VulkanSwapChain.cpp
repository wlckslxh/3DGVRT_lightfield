...
#endif
{
	...
	// To be edited. (by Sehee)
	// Check if the graphics queue used supports time stamps
	if (queueProps[queueNodeIndex].timestampValidBits == 0)
	{
...
//#endif
	...
	uint32_t desiredNumberOfSwapchainImages = std::max(MAX_FRAMES_IN_FLIGHT, surfCaps.minImageCount + 1);
	...
	imageCount = desiredNumberOfSwapchainImages;
