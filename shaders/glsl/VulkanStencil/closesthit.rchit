...
layout(binding = 11, set = 0) uniform uniformBufferStaticLight
{
	Light lights[numOfStaticLights];
} uboStaticLight;
layout(binding = 10) uniform usampler2D samplerAccumulation;
...
void main()
{
	...
	for(int i = 0; i < numOfLights; i++) {
		...
		if (pushConstants.rayOption.shadowRay) {
			...
			vec3 rayDirection = ubo.lights[i].position.xyz - tri.pos;
			...
			shadowed = true; 
#if ANY_HIT
			traceRayEXT(topLevelAS, gl_RayFlagsTerminateOnFirstHitEXT | gl_RayFlagsSkipClosestHitShaderEXT, 0xFF, 1, 0, 1, origin, RAY_TMIN, rayDirection, tmax, 1);
#else
			traceRayEXT(topLevelAS, gl_RayFlagsTerminateOnFirstHitEXT | gl_RayFlagsSkipClosestHitShaderEXT | gl_RayFlagsOpaqueEXT, 0xFF, 1, 0, 1, origin, RAY_TMIN, rayDirection, tmax, 1);
#endif
		}
		if (!shadowed) {
