...
layout(set = 0, binding = 8) uniform samplerCube pointShadowMap[numOfLights];
...
void main() 
{
    ...
    for(int i = 0; i < numOfLights; ++i)
    {
		...
		if (pushConstants.rayOption.shadowRay) {
			shadowed = CalculateShadow(pos - light.position.xyz, i);
		}
	...
	for (int iter = 0; iter < iterations; iter++)
	{
		...
		if (rayQueryGetIntersectionTypeEXT(rayQuery, true) == gl_RayQueryCommittedIntersectionTriangleEXT)  
		{
			...
			for(int i = 0; i < numOfLights; ++i)
			{
				...
				bool shadowed = false;
				if (pushConstants.rayOption.shadowRay) {
					shadowed = CalculateShadow(tri.pos - light.position.xyz, i);
				}
				if (!shadowed) {
